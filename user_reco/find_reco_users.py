...
class Neo4jPageRank:
    ...
    def create_pagerank_relationships(self, pagerank_results):
        ...
        """
        # Utiliser le seuil de configuration original
        ...
        with self._driver.session() as session:
            # Filtrer les nœuds au-dessus du seuil
            high_pagerank_nodes = {
            ...
            WHERE elementId(n) = node_id 
              AND (n.entity_type = 'user_preference' OR n.entity_type = 'positive_point' OR n.entity_type = 'activity')
            RETURN 
                elementId(n) AS id, 
            ...
            logging.info("\n--- Détails des Nœuds Potentiels au-dessus du seuil ---")
            for node in node_details:
                # N'afficher que les nœuds au-dessus du seuil
                if node['id'] in high_pagerank_nodes:
                    logging.info(f"ID: {node['id']}")
                    logging.info(f"Type d'entité: {node['entity_type']}")
                    logging.info(f"Labels: {node['labels']}")
                    logging.info(f"Description: {node.get('description', 'Pas de description')}")
                    logging.info(f"Propriétés du nœud: {dict(node['n'])}")
                    logging.info(f"Score PageRank: {high_pagerank_nodes[node['id']]['score']}")
                    logging.info("---")
            
            potential_relations = []
            for source in node_details:
                for target in node_details:
                    # Vérifier que les deux nœuds sont au-dessus du seuil et de types différents
                    if (source['id'] in high_pagerank_nodes and 
                        target['id'] in high_pagerank_nodes and 
                        ...
                        potential_relation = {
                            ...
                            'source_description': source.get('description', 'Pas de description'),
                            'source_properties': dict(source['n']),
                            'target_id': target['id'],
                            'target_entity_type': target['entity_type'],
                            'target_labels': target['labels'],
                            'target_description': target.get('description', 'Pas de description'),
                            'target_properties': dict(target['n']),
            ...
            logging.info(f"Nombre total de relations potentielles : {len(potential_relations)}")
            
            for relation in potential_relations:
                logging.info("Relation Potentielle:")
                logging.info(f"  Source - ID: {relation['source_id']}")
                logging.info(f"  Source - Type: {relation['source_entity_type']}")
                logging.info(f"  Source - Labels: {relation['source_labels']}")
                logging.info(f"  Source - Description: {relation['source_description']}")
                logging.info(f"  Source - Propriétés: {relation['source_properties']}")
                logging.info(f"  Cible - ID: {relation['target_id']}")
                logging.info(f"  Cible - Type: {relation['target_entity_type']}")
                logging.info(f"  Cible - Labels: {relation['target_labels']}")
                logging.info(f"  Cible - Description: {relation['target_description']}")
                logging.info(f"  Cible - Propriétés: {relation['target_properties']}")
                logging.info(f"  Score PageRank: {relation['pagerank_score']}")
                logging.info("---")
