    ...
    async def ainsert(self, string_or_strings, **kwargs):
        ...
        try:
            ...
            if update_storage and self.chunk_entity_relation_graph is not None:
                ...
                # Récupérer toutes les entités de type "activity"
                async def categorize_activities():
                    # Utiliser un LLM léger et rapide
                    use_model_func = self.global_config.get("llm_model_func")
                    
                    # Requête pour récupérer les entités "activity"
                    query = """
                    MATCH (n {entity_type: 'activity'})
                    RETURN n.id as id, n.description as description
                    """
                    
                    # Exécuter la requête sur le graphe
                    results = await self.chunk_entity_relation_graph.query(query)
                    ...
                    for result in results:
                        ...
                        # Si la catégorie est différente de "Autres", créer une relation
                        if category != "Autres":
                            # Créer ou récupérer le nœud de catégorie
                            category_node_query = f"""
                            MERGE (cat:ActivityCategory {{name: '{category}'}})
                            RETURN cat.id as category_id
                            """
                            category_result = await self.chunk_entity_relation_graph.query(category_node_query)
                            category_node_id = category_result[0]['category_id']
                            
                            # Créer la relation
                            relation_query = f"""
                            MATCH (activity {{id: '{activity_id}'}})
                            MATCH (category {{id: '{category_node_id}'}})
                            MERGE (activity)-[:BELONGS_TO]->(category)
                            """
                            await self.chunk_entity_relation_graph.query(relation_query)
