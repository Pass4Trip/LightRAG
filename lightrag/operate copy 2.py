...
async def extract_entities(
    ...
    for chunk_key, chunk in ordered_chunks:
        logger.info(f"Processing chunk: {chunk_key}")
        content = chunk.content
        
        # Obtenir les enregistrements de ce chunk
        try:
            prompt_template = entity_extract_prompt
            context = dict(
                content=content,
                examples=examples,
        ...
        for record in records:
            ...
            try:
                ...
                record_attributes = [attr.strip() for attr in record_attributes]
                logger.info(f"Record Attributes: {record_attributes}")
                
                if record_attributes[0].lower() == "entity":
                    entities_to_process.append((record_attributes, chunk_key))
                elif record_attributes[0].lower() == "relationship":
                    relationships_to_process.append((record_attributes, chunk_key))
                
            except Exception as e:
                logger.error(f"Error processing record: {e}")
                continue
    
    # Traiter toutes les entités d'abord
    for record_attributes, chunk_key in entities_to_process:
    ...
    # Ensuite traiter toutes les relations
    for record_attributes, chunk_key in relationships_to_process:
    ...
    if not len(entities_to_process) and not len(relationships_to_process):
        logger.warning(
            "Didn't extract any entities and relationships, maybe your LLM is not working"
        )
        return None

    if not len(entities_to_process):
        logger.warning("Didn't extract any entities")
    if not len(relationships_to_process):
    ...
    if relationships_vdb is not None:
        data_for_vdb = {
            ...
            for dp in relationships_to_process
        }
